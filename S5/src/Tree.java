...
public class Tree {
  ...
  public void computeMinimaxValues() {
    if (isLeaf()) {
      this.minimaxValue = new Triplet(true, this.state.getBluePoints(),
          this.state.getOrangePoints());
      return;
    }
    ...
    if (state.isBlueToPlay()) {
      this.minimaxValue = minBlue(leftChild.getMinimaxValue(), rightChild.getMinimaxValue());
    } else {
      this.minimaxValue = minOrange(leftChild.getMinimaxValue(), rightChild.getMinimaxValue());
    }
  }
